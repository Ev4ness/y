...
async def is_welcome_on(chat_id: int) -> bool:
    ...
    if not data:
        return False
    return data.get("welcome_on", False)
...
async def is_goodbye_on(chat_id: int) -> bool:
    ...
    if not data:
        return False
    return data.get("goodbye_on", False)


async def set_welcome_off(chat_id: int) -> bool:
    result = await greetingsdb.update_one(
        {"chat_id": chat_id},
        {"$set": {"welcome_on": False}},
        upsert=True
    )
    return result.modified_count > 0 or result.upserted_id is not None


async def set_welcome_on(chat_id: int) -> bool:
    result = await greetingsdb.update_one(
        {"chat_id": chat_id},
        {"$set": {"welcome_on": True}},
        upsert=True
...
async def set_goodbye_off(chat_id: int) -> bool:
    result = await greetingsdb.update_one(
        {"chat_id": chat_id},
        {"$set": {"goodbye_on": False}},
        upsert=True
    )
    return result.modified_count > 0 or result.upserted_id is not None


async def set_goodbye_on(chat_id: int) -> bool:
    result = await greetingsdb.update_one(
        {"chat_id": chat_id},
        {"$set": {"goodbye_on": True}},
        upsert=True
